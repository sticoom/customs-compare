...
| 32 | 单商品续页价格被吞进数量列/币制=目的国(页脚汇总数字污染价格列中位数) | `pdf_parser.py::extract_pre_recording_standard_vertical` 页脚过滤+原产国上界 | 2026-09-09 |
| 33 | 交叉校验放行单侧漏提(C2跨侧合并池+C1跳过无价item+续页未传入) | `table_crosscheck.py` C2按侧+C5镜像；`field_extractor.py` 调用点传续页 | 2026-09-09 |
| 34 | 横向页误判纵向(主页错续页对)+规格续段+20px穿列 | `pdf_parser.py::_find_horizontal_item_anchor` + `extract_pre_recording_items_horizontal` | 2026-09-11 |
...
- ⚠️ 已被 #34 收紧：同 x 否决须同时满足"值呈连续序号"——表头汇总值（毛净重/件数）也会同 x 三连但不是项号列，否则真横向页被误否决（20260910006 批次）。
...
### #29. 预录单幻影商品（件数+合同号被当项号+编码，如项号 428）
- **日期**：2026-09-07
- **现象**：20260904008（428箱）批次预录单侧多出一条幻影商品：`item_no=428`（实为件数）、`product_code=2026090400`（合同号 20260904008 被截断 10 位）、`product_name=纸制或纤维板制盒/箱`（实为包装种类值）、`spec_model` 是整段表头标签串、`unit_price=8`（垃圾）。比对 2 fail + 界面一大坨错位行。
- **根因**：仿报关单版式的核对单（MATS4612237001_khjy.pdf），页面头部值区文本顺序恰好为 `428(件数)\n20260904008(合同协议号)\n纸制或纤维板制盒/箱(包装种类)...`。`_extract_items_from_continuation` 的正则 `(\d{1,3})\s*\n\s*(\d{8,10})` 无右边界：(1) "件数\n合同号"相邻被当成"项号\n商品编码"；(2) 合同号 11 位数字被 `\d{8,10}` 从左侧截断吃前 10 位冒充编码。位置感知提取本身完全正确（只提 1 条真实商品），幻影是经 extract_all_fields 的"文本提取补充遗漏 item"安全网（field_extractor.py 997-1002 行）合入的。
- **修复**：编码组加 `(?!\d)` 右边界（含 lookahead 分隔符内同款）——11 位数字串无法满足"8-10 位且右邻非数字"，整个匹配自然失败。单点修在正则源头，customs 侧（906/930/945 行）与 pre 侧（978/986 行）所有调用点同时受益，无需改各处 merge 逻辑。
- **关键教训**：`\d{8,10}` 这类区间量词会**从长数字串左侧截断匹配**。报关单版式里"件数\n合同协议号"两值相邻是固定排版（件数总在合同号上一行），核对单/续页文本提取都会扫过这段——编码匹配必须带数字右边界，否则每个新批次都可能复活。与 #26 同类：几何/模式判据的巧合通道要用边界条件堵死。
- **影响**：`field_extractor.py::_extract_items_from_continuation`（1 处正则）。
- **验证**：20260904008 幻影项消失、2 fail→0（24 pass / 0 fail / 5 人工）；5 fixture 回归全绿（3 零变化 + 2 仅 trade_country 展示值修正，summary 不变）。
- **关联**：#12（item_no 文本/位置路径不一致产生重复项）、#26（判据巧合通道）——本坑是"文本兜底路径的巧合匹配"第三例。

### #30. 报关单贸易国提取为"(地区)"（半角标签变体漏映射 + 正则吃标签碎片）
- **日期**：2026-09-07
- **现象**：20260904008 报关单 `trade_country="(地区)"`（标签"贸易国(地区)"的后半截），预录单="(HKG)中国香港"。界面展示错位；此前 diagnose 未报异常是因为 fuzzy 蒙混通过。
- **根因**：两层叠加。(1) `extract_customs_header_by_grid` 的 LABEL_MAP 只收了全角 `"贸易国（地区）"`，漏了半角 `"贸易国(地区)"`——本批次报关单恰用半角括号（同页 `运抵国(地区)`/`毛重(千克)` 的半角变体都有映射，唯独贸易国漏了），grid 漏提；(2) dispatch 判 grid 命中（sender_unit/contract_no 有值）后用 `extract_customs_header` 文本正则补遗漏字段，其正则 `贸易国\s*\n?\s*(.+)` 在网格排版下捕获的是标签自己/下一标签的尾巴——标签行后跟的是另一标签（"贸易国(地区)\n运抵国(地区)..."），值块在更下方，文本正则结构上无法配对。
- **修复**：(1) LABEL_MAP 补 `"贸易国(地区)": "trade_country"`（根修）；(2) 正则捕获含"地区"的值时放弃（防御）——贸易国合法值（中国香港/(HKG)中国香港/美国等）不含"地区"，宁留空交给 grid，不再产出垃圾。
- **关键教训**：标签变体要**全半角成对收**——同一张 PDF 里半角/全角括号混用（"贸易国(地区)" vs "毛重（千克）"），LABEL_MAP 收录时逐条核对实页 span 文本，不能举一反三 assumes。fuzzy 匹配容忍垃圾值会让提取 bug 沉默化（"(地区)" vs "(HKG)中国香港" 被判 pass），golden 基线应固化干净值让这类问题显形。
...
### #32. 单商品续页价格列被页脚汇总数字污染（列中位数错位，整行字段错乱）
- **日期**：2026-09-09
- **现象**：20260909001 批次（935箱）预录单项号 8（单商品续页 Page1）整行错位：单价 47.2600 / 总价 14461.56 / 币制"人民币"被吞进 `quantity_unit`（"847千克 / 47.2600 / 人民币 / 14461.56 / 306件"），"美国"塞进 `currency`，`final_dest_country`/`domestic_source` 空，比对 4 字段异常；且交叉校验 0 告警（见 #33）。同文件 Page0 的 7 条商品全部正常。
- **根因**：`extract_pre_recording_standard_vertical` 的"价格列数据修正"块用数据 span 的 x **中位数**定位价格列，但候选池只过滤了 `y > header_y+5`，**没排除页脚**——页脚的「表体总净重/总毛重/总件数」（14851/15786/935）是纯数字 span（x≈614-615），天然匹配 PRICE_RE，且恰好在真实价格列（x≈463-472）与货源地列之间。多商品页真实价格 span 占多数，中位数无恙；单商品续页只有 2 个真实价格 span，候选池 `[463, 467.5, 614, 615, 615]` 的中位数=614——价格列被推到目的国/货源地列之间，从数量列起全列边界错位。
- **修复**：`pdf_parser.py::extract_pre_recording_standard_vertical`：
  1. 页脚检测（footer_y，关键词与主提取同一套）从主提取段**提前**到列推断之前，价格列推断块与数据修正块的候选池统一加 `y < footer_y-2` 过滤；
  2. 价格候选加**几何上界**：真实价格列必在原产国列左侧（`x < origin_x - 10`；原产国与价格共享表头 x 时不设界），页脚数字天然在原产国右侧、被结构性地排除；
  3. 主提取的页脚过滤复用前置的 footer_y（逻辑不变，消除重复）。
- **关键教训**：**单商品/短续页是中位数类启发式最脆弱的场景**——固定数量的噪声（页脚汇总数字永远存在）在候选数少时反超真实数据。列位置推断的候选池必须与主提取同等地排除页脚；「某列必在相邻列左侧」的几何不变量比"数值长得像价格"更可靠。修复后候选 `[463, 467.5]` 中位数回到真实列。
- **影响**：`pdf_parser.py::extract_pre_recording_standard_vertical`（页脚检测前置 + 2 处候选池过滤 + 1 处上界）；主提取行为不变。
- **验证**：20260909001 项号 8 十二字段全对（unit_price=47.2600、total_price=14461.56、final_dest=美国 (USA)、domestic_source=惠州其他、currency=人民币），汇总 89→93 pass / 2→1 fail / 1→0 fuzzy；6 pair + 2 single 回归零变化。
- **关联**：#10（row_slots 不足丢 item）同为"单商品/少线续页"脆弱场景；#21/#22（列边界漂移）同函数族；#26（残缺结果被安全网采用）是同一页族（20260902003）的前车之鉴。

### #33. 架构：交叉校验 C2 按侧分池 + C5 两侧镜像（补上 V1 校验层的单侧漏提盲区）
- **日期**：2026-09-09
- **现象**：#32 的错乱发生时交叉校验 **0 告警**，三重放行：(1) C1 对缺单价/总价的 item 直接 `continue`——错乱恰好把价格吞掉了，于是跳过；(2) C2 把两侧所有页面单元格**合并成一个池**，预录侧编码在报关单侧单元格中出现即通过；(3) `extract_all_fields` 调用校验层时只传 `pre_pages`，**预录单续页（unknown 类型）根本不在校验视野**——项号 8 所在页没被任何检查覆盖。
- **根因**：V1 校验层（#31）以"值在不在"为判据且池子跨侧，没有"本侧完整性"概念。单侧漏提/错位类错误（#26 丢 12 条、#32 价格被吞）在单侧视角下完全合法，只有拿**另一侧做镜像**才显形。
- **修复**：`src/table_crosscheck.py` + `src/field_extractor.py` 调用点：
  - **C2 按侧分池**：编码须在「本侧」单元格 ∨ 本侧页面原始文本（均词边界 `\b`）出现。本侧文本兜底是**必须**的——find_tables 对部分预录单格式抓不到编码列（20260909001 实测 4/4 编码不在本侧单元格），纯单元格判据会系统性误报（违背 #31"校验层怕误报"教训）；而截断码（732399000）/截断合同号（2026090400）在本侧长数字串中间取不到词边界，仍会报警。
  - **C5 两侧镜像（新增）**：项号集合不对称 → 报"疑漏提/幻影"；同项号一侧有单价/总价另一侧全空 → 报"疑漏提或错位"。
  - **调用点传入续页**：`crosscheck_extraction(customs_pages, pre_pages + pre_continuation_pages, ...)`，续页商品纳入 C2/C5 视野。
- **关键教训**：校验层的池子边界要和"错误归属"对齐——跨侧合并池会让"另一侧恰好有同值"的单侧错误静默通过。镜像校验是发现单侧漏提的最廉价手段：对侧就是同一份申报的独立提取，天然是参照物。C5 与 C1 互补：C1 查"价格数值对不对"，C5 查"价格在不在"。
- **影响**：`src/table_crosscheck.py`（C2 重写 + C5 新增）、`src/field_extractor.py::extract_all_fields`（调用点传续页）。
- **验证**：8 样本回归（含新登记的 20260902003/20260904008/20260909001 三个故障批次）——正确提取 0 新告警；pair_20260717011 出现 1 条 C2 **真阳性**（#23 遗留的预录单项号21 编码 732399000 截断，此前沉在比对 fail 里，现在校验层也显形）；注入复现 #32 错乱结果，C5 精准命中「预录单项号8价格字段缺失」。
- **关联**：#31（校验层 V1，本坑是其盲区首例实战）；#26/#32（被放行的两类单侧错误）；#29（\b 词边界双保险的前身）。
- **同批工程改进**：回归样本从 5 扩到 8——9 月三个故障批次（20260902003/20260904008/20260909001）登记进 `tests/regress.py`，`snapshot_pair` 新增固化 `warnings`（校验层自身的意外增减也会显形）。新故障批次应随修复同步登记，"修完不留样本"是复发根源之一。

### #34. 横向核对单整页误判纵向（表头毛净重/件数同 x 三连误触发同 x 否决）+ 规格续段 +20px 穿列
- 日期：2026-09-11
- 现象：20260910006（19箱 DLM250901+DLM250904 合并，J18632B 系列）预录单 Page0（项号 1-6，横向倒排）被分类器判成 standard_vertical，6 条商品全错位：编码被截成 9 位（420292000）、规格串混入 "0|1|2|..." 前缀、数量跨 item 拼接（"12千克 / 30套 / 8千克 / 30套 / 35千克"）、项5/6 整条缺失、项4 价格缺失。比对 23 fail + C2/C5 告警 6 条。同批"主页全错、续页（项7-9）全对"的割裂现象是本坑的指纹。
- 根因：两层。
  - (1) #26 的同 x 否决扫描**全页所有纯整数 span**——本页表头汇总值 净重228/毛重247/件数19 恰好同在 x=193.5（申报表头值列），构成"≥3 整数同 x"假象，真横向页被否决。#26 的判据语义是"项号列全在同 x"，但实现没限定输入语义：表头值列（毛净重/件数）天然同 x 却不是项号。Page1 无表头数值区、判定正确，恰证否决输入不纯。
  - (2) 分类修复后暴露第二层：规格续段（如 "|无款号|||2个装"）与 #23 的数量一样带 **+20px 系统性 x 右偏**（项号x+20.5 > 列中点边界=项号x+16），穿列落到下一 item——项4 混入项3 的续段、项5 混入项4 的续段。fuzzy 比对容忍了污染（#30 教训复现）；pair_20260717011（125箱）同源污染也一直被 fuzzy 沉默着 12 条。
- 修复：`pdf_parser.py`
  - `_find_horizontal_item_anchor` 同 x 否决加"值呈连续序号"条件：同 x 组去重后 ≥3 个值且相邻差 ∈{1,2} 才判为项号列；19/228/247 互不连续，不触发否决。
  - `extract_pre_recording_items_horizontal` 规格提取从"列内收集"改为**页宽 span 重归位**：含 "|" 的 span 复用数量同款项号 x 区间 `[cx-2, next_cx-2)`（`_qty_owner`）归列，再按 y 升序（自上而下阅读序）拼接——主规格在前、续段在后，续段回到正确 item。
- 关键教训：
  - 几何否决判据的输入必须限定语义——"整数同 x" ≠ "项号列"。区分特征在**值序列**：项号是连续小序号（8,9,10…），表头汇总值互不连续（19/228/247）。几何判据永远可能巧合，值特征是更本质的判别量。
  - +20px 渲染右偏是这类 PDF 生成器的**系统性现象**，已有两种表现（#23 数量、本次规格续段）。凡"列内多段字段"（一个字段拆多个 span 渲染在不同 y）都要怀疑偏移穿列，归位一律用项号 x 区间，不用列中点。
- 影响：`pdf_parser.py::_find_horizontal_item_anchor`（否决收紧）、`extract_pre_recording_items_horizontal`（规格重归位 + y 升序拼接）；顺带修复 pair_20260717011 的 12 条被 fuzzy 沉默的同源污染（fuzzy 12→1，pass 209→220）。
- 验证：本样本 23 fail→0（97 pass / 0 fail / 7 fuzzy / 5 manual，C2/C5 告警全消）；9 fixture 回归全绿（含重建后的 pair_20260717011 golden）；故障批次已登记 `pair_20260910006`（pass=97, fail=0）。
- 关联：#26（否决判据的收紧，非推翻）、#23（+20px 偏移首例——数量）、#21/#22（同函数族列边界问题）、#30（fuzzy 沉默化教训的又一次应验）。
