...
from src.config import CUSTOMS_ITEM_DISPLAY_ORDER
...
def render_comparison_result(result, idx):
    ...
    # ---- 比对明细（按报关单原始顺序，不按状态分组）----
    st.markdown(f'<div style="font-weight:600;font-size:15px;margin-top:16px;">📋 比对明细</div>',
                unsafe_allow_html=True)
    ...
    for r in result["header_results"]:
        ...
        header_data.append({
            "状态": render_status_icon(r["status"]),
            "字段名称": r["label"],
    ...
    for item_result in result["item_results"]:
        st.markdown(f"**项号 {item_result['item_no']}**")

        # 按 config 中的展示顺序重排字段
        fields_by_id = {f["field_id"]: f for f in item_result["fields"]}
        item_data = []
        for field_id in CUSTOMS_ITEM_DISPLAY_ORDER:
            if field_id in fields_by_id:
                ...
                item_data.append({
                    "状态": render_status_icon(f["status"]),
                    "字段名称": f["label"],
